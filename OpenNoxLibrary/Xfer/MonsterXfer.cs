...
namespace OpenNoxLibrary.Xfer
{
	...
	public class MonsterXfer : DefaultXfer
	{
		...
		public Monsters.StatusFlags StatusFlags; // статус флаги
		...
		public Color24[] MaidenBodyColors;
		...
		public struct SpellEntry
		{
			...
			public Monsters.SpellCastFlags UseFlagsEnum
			{
				get
				{
                    return (Monsters.SpellCastFlags)UseFlags;
				}
			}
			...
			public override string ToString()
			{
                if (ThingDb.SpellNames.Contains(SpellName))
                {
                    ThingDb.Spell spell = ThingDb.Spells[ThingDb.SpellNames.IndexOf(SpellName)];
                    string result = spell.DisplayNameCsf;
                    int e = result.IndexOf(':') + 1;
                    result = result.Remove(0, e);
                    return result;
                }
                return String.Format("{0}:{1}", SpellName, UseFlagsEnum);
			}
		}
		...
		private void InitDefault()
		{
			...
			StatusFlags = (Monsters.StatusFlags) 0;
			HealthMultiplier = 1F;
			RetreatRatio = 0.05F;
			ResumeRatio = 0.5F;
			SightRange = 300F;
			...
			MaidenBodyColors = new Color24[6];
		...
		public void InitForMonsterName(string monsterID)
		{
			...
			if (isFemaleNPC)
			{
				for (int i = 0; i < 6; i++) MaidenBodyColors[i] = new Color24(0xD2, 0xAE, 0x79);
			}
		...
		public override bool FromStream(Stream mstream, short ParsingRule, ThingDb.Thing thing)
		{
			...
			if (ParsingRule >= 31)
			{
				...
				if (ParsingRule < 51)
					StatusFlags = (Monsters.StatusFlags) br.ReadUInt16();
				else
                    StatusFlags = (Monsters.StatusFlags)br.ReadUInt32();
				...
				if (ParsingRule >= 33)
				{
					...
					if (ParsingRule < 53)
					{
                        ...
                        TrapSpell1 = ThingDb.Spells[spellIndex].SysName;
						spellIndex = br.ReadInt32();
						TrapSpell2 = ThingDb.Spells[spellIndex].SysName;
						spellIndex = br.ReadInt32();
						TrapSpell3 = ThingDb.Spells[spellIndex].SysName;
					}
					else
					{
				...
				if (ParsingRule >= 34)
				{
					...
					DefaultAction = Array.IndexOf(Monsters.ActionStrings, action);
				}
				if (ParsingRule >= 41)
				{
				...
				if (ParsingRule >= 54 && thing.Subclass[(int) ThingDb.Thing.SubclassBitIndex.FEMALE_NPC])
				{
					...
					for (int i = 0; i < 6; i++)
					{
						...
						G = br.ReadByte();
						B = br.ReadByte();
						MaidenBodyColors[i] = new Color24(R, G, B);
					}
					if (ParsingRule >= 55)
		...
		public override void WriteToStream(Stream mstream, short ParsingRule, ThingDb.Thing thing)
		{
			...
			bw.Write(TrapSpell2);
			bw.Write(TrapSpell3);
			bw.Write(Monsters.ActionStrings[DefaultAction]);
